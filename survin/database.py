...
db.execute("CREATE TABLE IF NOT EXISTS classifications (path TEXT, classification TEXT)")
db.execute("CREATE INDEX IF NOT EXISTS idx_classification_path ON classifications (path)")
db.commit()

def add_file(path: Path):
    db.execute("INSERT INTO files (path) VALUES (?)", (str(path),))
    db.execute("UPDATE files SET status = ? WHERE path = ?", (Status.NEW.name, str(path)))
    db.commit()

def set_classifications(path: Path, classifications: set[str]):
    for classification in classifications:
        db.execute("INSERT INTO classifications (path, classification) VALUES (?, ?)", (str(path), classification))
    db.commit()
...
def get_files(status: Status) -> list[Path]:
    return [Path(row[0]) for row in db.execute("SELECT path FROM files WHERE status = ?", (status.name,))]
